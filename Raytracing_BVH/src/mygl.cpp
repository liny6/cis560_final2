...
#include <raytracing/samplers/imwidepdsampler.h>
#include <tbb/tbb.h>
#include <QTime>
...
//#define USEIMWIDE
#ifndef USEIMWIDE
    StratifiedPixelSampler sampler(num_samples);
#else
    ImWidePDsampler sampler(num_samples);
#endif
...
//    #define TBB //Uncomment this line out to render your scene with multiple threads.
    //This is useful when debugging your raytracer with breakpoints.
        ...
        //timer1.stop();
#else
    QList<glm::vec2> points(sampler.GetSamples(scene.camera.width, scene.camera.height));
    float x_pixel(0);
    float y_pixel(0);

    for(int i = 0; i < points.count(); i++)
    {
        Ray r(scene.camera.Raycast(points[i].x, points[i].y));
        color = integrator.TraceRay(r, 0);
        //round sample point
        x_pixel = glm::clamp(glm::round(points[i].x), 0.0f, static_cast<float>(scene.camera.width));
        y_pixel = glm::clamp(glm::round(points[i].y), 0.0f, static_cast<float>(scene.camera.height));

        scene.film.pixels[static_cast<int>(x_pixel)][static_cast<int>(y_pixel)];
    }
#endif
    #endif
